...
<body>
	...
	<div id="wrapper">
		...
		<!-- 로그인을 안한 경우 --> <!-- 로그인 한 경우 --> <!-- 모든 계정 --> <!-- a태그 : 로그아웃 / 개인정보수정 -->
		<!-- 관리자 계정 --> <!-- a태그 : 전체회원정보 --> <c:choose>
			<c:when test="${empty loginM }">
				<a href="#menu">로그인</a>
			</c:when>
			<c:otherwise>
				<c:if test="${loginM.email eq 'admin'}">
					<a href="/message/member/selectAllMember">전체회원정보</a>
				</c:if>
		...
		<div class="inner">
			<header class="major"> <!-- 로그인을 안한 경우 --> <!-- 로그인 한 경우 -->
			<!-- h1 : ~(이메일)님 환영합니다 --> <!-- 로그인 후 로그인 한 사용자의 세션아이디로 바꾸시오.
			ex)smart님 환영합니다 --> <c:choose>
				<c:when test="${empty loginM}">
					<h1>로그인 한 세션아이디를 출력해주세요</h1>
				</c:when>
				<c:otherwise>
					<h1>${loginM.email}님환영합니다</h1>
				</c:otherwise>
			</c:choose> </header>
			<div class="content">
				<p>
			...
			<div class="inner">
				...
				<ul class="actions">
					<c:choose>
						...
						<c:otherwise>
							<li><a href="/message/mail/deleteAllMail/"
								class="button next scrolly">전체삭제하기</a></li>
						</c:otherwise>
					</c:choose>
				</ul>
				<table>
					<c:forEach items="${messageList }" var="mail" varStatus="status">
						<tr>
							<td>${status.count}</td>
							<td>${mail.sendName}</td>
							<td>${mail.message}</td>
							<td>${mail.sendDate}</td>
							<td><a href="/message/mail/deleteMail/${mail.num }">삭제</a></td>
						</tr>
					</c:forEach>
				</table>
				<div>
					<ul class="pagination">
						...
						<c:forEach begin="${pagination.startPage}"
							...
							<li
								class="page-item ${pagination.cri.pageNum == idx ? 'active' : ''}">
					...
					<form id="pageForm" action="/message/main">
						...
						<input type="hidden" name="amoutn" value="${pagination.cri.amount }">
					</form>
		...
		<div class="inner">
			...
			<form action="/message/mail/sendMail" method="POST">
				<div class="field half first">
					<label for="name">Name</label> <input type="text" name="sendName"
						id="name" placeholder="보내는 사람 이름" />
				</div>
				<div class="field half">
					<label for="email">Email</label> <input type="text"
						name="receiveEmail" id="email" placeholder="보낼 사람 이메일" />
				</div>
	...
	<script>
		...
		$(".page-link").on("click",function(e){
			e.preventDefault(); // a태그의 기능 막기(이동막기)
